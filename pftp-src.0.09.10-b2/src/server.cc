...
CServer::CServer()
{
    ...
    if (passive_default)
        this->pasv_listing = TRUE;
    else
        this->pasv_listing = FALSE;
...
#endif
    case E_BAD_USER:
        sprintf(temp_string, "[%s]: %s", prefs.label, E_MSG_BAD_USER);
        break;

    case E_BAD_PASS:
        sprintf(temp_string, "[%s]: %s", prefs.label, E_MSG_BAD_PASS);
        break;
    ...
    do {
        ...
        if (!this->is_transfering) {
            ...
            if (this->prefs.use_noop) {
                if (this->noop_slept >=
                    (SLEEPS_NEEDED * this->prefs.noop_rate)) {
                    this->is_busy = TRUE;
                    this->noop_slept = 0;
                    if (!this->Noop())
                        this->EvalError();
...
#ifdef TLS
    if (trytls(prefs.label)) {
        this->PostBusy("AUTH");

        sprintf(this->temp_string, "AUTH TLS\r\n");
        if (!this->tcp.SendData(this->temp_string)) {
            this->error = E_CONTROL_RESET;
            this->PostBusy(NULL);
            return (FALSE);
	    ...
	    nossl=1;
            ...
            if (!this->tcp.SecureControl()) {
                ...
                sprintf(this->temp_string, "[%s]: %s", this->prefs.label,
                        E_MSG_BAD_TLSCONN);
                display->PostStatusLine(this->temp_string, TRUE);
            } else {
                sprintf(this->temp_string,
     ...
     sprintf(this->temp_string, "[%s]: NOTLS site... Punishing the site with 5 seconds sleep :)",
                this->prefs.label);
     display->PostStatusLine(temp_string, FALSE);
     sleep(5);
     nossl=1;
...
/*    while (sv_temp) {
    ...
    if (!SetProt(0)) return (FALSE); //secure data off for fxp
    if (!dest->SetProt(0)) return (FALSE); //secure data off for fxp
...
//TODO :glbug
    ...
    if (size == -1) {
        sprintf(this->temp_string, "[%s]: uploaded '%s' (%ld seconds)",
                dest->prefs.label, file,
        ...
        speed = ((float) size / ((float) (speed) / 1000000.0)) / 1024;
        sprintf(this->temp_string,
                "[%s]: uploaded '%s' (%ld) at %4.02f kb/s (%ld seconds)",
                dest->prefs.label, file, size, speed,
                (long) (after.tv_sec - before.tv_sec));
    } else {
        sprintf(this->temp_string,
                "[%s]: uploaded '%s' size : %ld (%ld seconds)",
                dest->prefs.label, file, size,
                (long) (after.tv_sec - before.tv_sec));
    }
    display->PostStatusLine(this->temp_string, FALSE);
...
bool CServer::CheckStealth(char *filename)
{
    ...
    if ((file = fopen(filename, "r"))) {
        ...
        if (strstr(buffer, "total "))
            ...
            sprintf(this->temp_string,
                    "[%s]: unable to enable stealth mode, using std. mode",
                    this->prefs.label);
            display->PostStatusLine(this->temp_string, TRUE);
            remove(filename);
            return (FALSE);
...
//    snprintf(temp_file,1023, "%spftpfxpXXXXXX", okay_dir);
    ...
    snprintf(temp_file,1023, "%spftpfxp-%d", okay_dir,getpid());
...
void CServer::SortFilelist(bool add_root)
{
    ...
    FILELIST *fl_temp, *fl_temp1, *fl_new, *fl_temp2, *fl_best,
        *fl_best_before;
    int magic;
        ...
        while (!finished) {
            ...
            while (fl_temp1) {
                ...
                case SORTING_METHOD_ALPHA:
                    if (strcasecmp(fl_temp1->name, fl_best->name) < 0) {
                ...
                case SORTING_METHOD_SIZE:
                    if (fl_temp1->size > fl_best->size) {
                        fl_best_before = fl_temp2;
                        fl_best = fl_temp1;
                    } else {
                        if (fl_temp1->size == fl_best->size) {
                            if (strcasecmp(fl_temp1->name, fl_best->name) <
                                0) {
                                fl_best_before = fl_temp2;
                                fl_best = fl_temp1;
                            }
            ...
            strcpy(fl_new->group, fl_best->group);
...
//      fl_temp2 = this->actual_filelist;
    ...
    while (fl_temp) {
        if ((!fl_temp->is_dir)
            &&
            (wild_match
             ((unsigned char *) "*.sfv", (unsigned char *) fl_temp->name)))
        {
            fl_temp1->next = fl_temp->next;
