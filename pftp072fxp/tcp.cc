...
bool CTCP::WaitForMessage(int timeout)
{
	...
	do {
		...
		do {
			while(((read_length = this->WaitForDataAndRead(TRUE, already_read)) == -1) && ((timeout == 0) || (time < timeout))) {
				time++;
			}
...
bool CTCP::OpenData(char *port_str, bool use_local)
{
	...
	if(getsockname(this->data_sock_fd, (struct sockaddr *)&data_sock_in, (socklen_t *)&sockaddr_len) < 0) {
		close(this->data_sock_fd);
		this->error = E_SOCKET_BIND;
		return(FALSE);
	}
...
bool CTCP::AcceptData(void)
{
	...
	do {
		this->real_data_sock_fd = accept(this->data_sock_fd, (struct sockaddr *)&(this->data_sock_in), (socklen_t *)&sockaddr_len);
		n++;
