...
void mywborder(WINDOW *win) {
    wborder(win, ACS_VLINE| COLOR_PAIR(STYLE_NORMAL), 
     ACS_VLINE| COLOR_PAIR(STYLE_NORMAL), ACS_HLINE| COLOR_PAIR(STYLE_NORMAL),
      ACS_HLINE| COLOR_PAIR(STYLE_NORMAL),  ACS_ULCORNER| COLOR_PAIR(
      STYLE_NORMAL),  ACS_URCORNER| COLOR_PAIR(STYLE_NORMAL), 
       ACS_LLCORNER| COLOR_PAIR(STYLE_NORMAL), ACS_LRCORNER| 
       COLOR_PAIR(STYLE_NORMAL));
}
...
void FireupLocalFilesys(void)
{
    ...
    bm.use_pasv = bm.use_ssl = bm.use_ssl_list = bm.use_ssl_data = FALSE;
    bm.sorting=1;
