...
CServer::CServer() {
    ...
    this->sort_method = 1;      // default sort
        ...
        if (!this->is_transfering) {
            ...
            if (this->prefs.use_noop) {
                if (this->noop_slept >=
                    ...
                    if (prefs.use_rndrefr) {
                        random_add_noop = (int) (-this->prefs.noop_rate / 2 +
...
bool CServer::Login(void) {
    ...
    for (n = 0; n < prefs.retry_counter + 1; n++) {
        ...
        if (prefs.use_retry && no_login) {
            sprintf(this->temp_string, "[%s]: could not log in, will retry in %d seconds", prefs.label,prefs.retry_delay);
            display->PostStatusLine(this->temp_string, FALSE);
            this->tcp.CloseControl();
    ...
    if (!this->tcp.WaitForMessage()) {
        sprintf(this->temp_string, "[%s]: TYPE set failed", this->prefs.label);
        display->PostStatusLine(temp_string, TRUE);
        this->PostBusy(NULL);
    ...
    if (prefs.use_firstcmd && prefs.first_cmd) {
        ...
        if (!this->tcp.WaitForMessage()) {
            sprintf(this->temp_string, "[%s]: 1ST CMD failed", this->prefs.label);
            display->PostStatusLine(temp_string, TRUE);
            this->PostBusy(NULL);
        }
    }
