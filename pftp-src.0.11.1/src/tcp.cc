...
void CTCP::UpdateStack() {
    ...
    //hm hope this works ok, it overwrites itself sometimes
    if (*start!='\0')  {
     strcpy(this->control_buffer,start); 
     already_read=strlen(this->control_buffer);
     debuglog("in control buffer left = %d bytes = \"%s\"",already_read,this->control_buffer);
    } else{
     already_read=0;
        ...
        if (len == -1) {
            ...
            if (ctrl_con) {
                ...
                case SSL_ERROR_WANT_X509_LOOKUP:
                    return 0; //means we should try ssl_read again
                    break;
                case SSL_ERROR_WANT_READ:
                    break;//good this is EAGAIN :)
                default:
                    return 1;
                    //fatal
                    break;
...
bool CTCP::WaitForMessage(int timeout) {
	...
	if (CONTROL_BUFFER_SIZE - already_read<=10) { //10 bytes of 64kb buffer
        ...
        do {
            ...
            pthread_testcancel();
            while (((result =
                         this->WaitForDataAndRead( already_read,&read_length)) == 0)
            ...
            if (read_length == 0) { //like wtf, can this EVER happen ?
            ...
            //like wtf again, this means we shoudl get \r\n as last chars
            //which is bullshit
